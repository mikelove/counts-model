...
We will use the *Salmon* software for estimating transcript abundance [@Patro2017].
...
We next set the `levels` of the `line` and `condition` factor variables
so that `OCT4` and `untrt` are the reference levels, and we specify
the path to the quantification files we want to read in.

In the last step of the `mutate` call, note we specify paths to the
quantification files `quant.sf.gz`. Typically, these files are not
gzipped, but we have compressed the `quant.sf` files to reduce the 
size of the data package. Note also that, although we point only to the
...
All the files exist at the locations we specified, which in this case
...
We will now use the Bioconductor package *tximeta* [@Love2019] to 
...
A small note: we specify `dropInfReps=TRUE`, because we will be
performing gene-level analysis, and we won't make use of the
uncertainty information in this first analysis. We will show 
...
object, we can obtain additional information, such as the 
mapping of transcripts to genes. We can then perform a summarization
...
Below we show some examples of operations that can be easily
performed because we have a *SummarizedExperiment* object
with the appropriate gene ranges attached. We can easily
...
We can likewise easily subset the samples by referring to 
...
Finally, we demonstrate that it is easily to add alternative
identifiers, making use of the organism annotation packages
in Bioconductor. For example, to add gene symbols, we can 
