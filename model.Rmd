...
formula expressing how we wish to model the counts. Here, we specify a
...
After creating the dataset, we perform some minimal filtering that
makes sense for bulk RNA-seq. We insist that there be at least three
samples with a count of 10 or more to keep the gene in the
...
called, ``The DESeq2 model''.

```{r deseq, cache=TRUE}
dds <- DESeq(dds)
```

One of the key estimates is the dispersion for each gene. Here I plot
the estimates over the mean of scaled counts for each gene. Note that
many of the plots in *DESeq2* refer to "normalized counts"; here this
just implies scaling the counts by the size factor, so that the 
library size differences across samples are minimized.
...
function to compute a variance stabilizing transformation of the count
data. This is similar to a log2 transform but avoids inflating the
variance of the low count genes. For more details on the methods used
here to compute the transformation, consult the *DESeq2* vignette or
`?vst`. We specify `blind=FALSE` which will use the sample grouping 
information when calculating the per-gene dispersion. The 
transformation itself does not use the design, once the global 
dispersion information has been fit.
...
We can extract various results tables from the `dds` object, based
...
say 0.1, provide a set that is expected to control a corresponding 
false discovery rate (FDR), for example no more than 10% of the genes
in the set should be false positives.
...
For example, ordering adjusted p-values from small to large (but then
remember that this object is no long aligned with `dds` for example).
...
MA-plot. But first, I will compute a new estimate of fold change.
The estimate in the results table is the MLE, or maximum likelihood
estimate, which is highly variable for low count genes (as we saw
in the simulated example). Here I compute a Bayesian estimate for the
...
identifiers, such as gene symbols to the MA-plot. We can select a 
subset of genes to add labels, here we filter the results table to a 
set of genes based on `baseMean` and `log2FoldChange` (these choices
are arbitrary, mostly for demonstration). For a given experiment, it 
...
re-ordered the `lfc` results table, I can add the `SYMBOL` column
to the table, and then create a smaller table with our genes of 
interest.
...
We now highlight the genes in `tab` using the `points` and `text` 
functions.
...
overlapping points below the horizontal axis. We can make a nicer
looking plot using *ggplot2*. First we create a data frame that we 
...
To use *IHW* instead of mean count thresholding, we pass the `ihw`
...
methods used here, consult the *IHW* package vignette and the methods
